...
def profile():
    ...
    if request.method == "GET":
        if session.get("UserID") is None:
            ...
            # for displaying all the car the user has added
                ...
                forFullPrice = cur.execute("SELECT MietStartdatum, MietEnddatum, PreisProTag FROM User LEFT JOIN Autobesitzer ON User.UserID = Autobesitzer.User LEFT JOIN Autos ON Autobesitzer.Auto = Autos.AutoID LEFT JOIN Mietauftrag ON Autobesitzer.User = Mietauftrag.Vermieter WHERE Mieter = (?)", [(session["UserID"])]).fetchall()
                if forFullPrice:
                    fullPrice = (datetime.strptime(forFullPrice[0][1], "%Y-%m-%d") - datetime.strptime(forFullPrice[0][0], "%Y-%m-%d")).days * forFullPrice[0][2]
                rented = cur.execute("SELECT AutoID, Hersteller, Modell, Fahrzeugtyp, (?) , MietStartdatum, MietEnddatum, Straße, Hausnummer, Postleitzahl, Ort, Benutzername FROM User LEFT JOIN Autobesitzer ON User.UserID = Autobesitzer.User LEFT JOIN Autos ON Autobesitzer.Auto = Autos.AutoID LEFT JOIN Adresse ON Adresse.AdressID = User.Adresse LEFT JOIN Mietauftrag ON Autobesitzer.User = Mietauftrag.Vermieter WHERE Mieter = (?)", [(fullPrice), (session["UserID"])]).fetchall()
                userRented = cur.execute("SELECT Auto FROM Mietauftrag WHERE Vermieter = (?)", [(session["UserID"])]).fetchall()
                print(rented)
    ...
    return render_template("profile.html", email = email, username = username, firstName = firstName, lastName = lastName, street = street, houseNum = houseNum, city = city, postalCode = postalCode, credit = credit, password = password, cars = cars, rented = rented, userRented = userRented)
...
def findCar():
    ...
    if session.get("UserID") is None:
        ...
        with sqlite3.connect("database.sqlite") as con:
            # for displaying all the car the user has added
                cur = con.cursor()
                cars = cur.execute("SELECT AutoID, Hersteller, Modell, Fahrzeugtyp, PreisProTag, Startdatum, Enddatum, Straße, Hausnummer, Postleitzahl, Ort, Benutzername FROM User LEFT JOIN Autobesitzer ON User.UserID = Autobesitzer.User LEFT JOIN Adresse ON User.Adresse = Adresse.AdressID LEFT JOIN Autos ON Autobesitzer.Auto = Autos.AutoID WHERE Autobesitzer.User != (?)", [(session["UserID"])]).fetchall()
        if request.method == "POST":
            ...
            with sqlite3.connect("database.sqlite") as con:
                ...
                if request.form.get("filter") == "Suchen":
                    ...
                    for i in range(len(cars)):
                        if request.form.get(str(cars[i][0])) == "mieten":
                            print("selected car")
                            lessor = cur.execute("SELECT User FROM Autobesitzer WHERE Auto = (?)", [(cars[i][0])]).fetchone()[0] #change 30 to variable
                            startDate = request.form["rent-startdate"]
                            endDate = request.form["rent-enddate"]
                            pricePerDay = cur.execute("SELECT PreisProTag FROM Autos WHERE AutoID = (?)", [(cars[i][0])]).fetchone()[0] #change 30 to variable
                            ammountDays = cur.execute("SELECT JULIANDAY(Enddatum) - JULIANDAY(Startdatum) AS difference FROM Autos WHERE AutoID = (?)", [(cars[i][0])]).fetchone()[0]
                            ...
                            cur.execute("INSERT INTO Mietauftrag(Mieter, Vermieter, Auto, Gesamtpreis, MietStartdatum, MietEnddatum, Ueberweisungsdatum) VALUES(?, ?, ?, ?, ?, ?, ?)", ((session["UserID"]), (lessor), (cars[i][0]), (endPrice), (startDate), (endDate), (startDate)))
